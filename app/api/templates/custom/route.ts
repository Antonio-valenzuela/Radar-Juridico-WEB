      ...
      if (rawStructure) {
        try {
          ...
          if (structureJson && typeof structureJson === 'object') {
            if (!Array.isArray(structureJson.pages) && !Array.isArray(structureJson.blocks)) {
              console.warn('[templates/custom] structureJson lacks pages/blocks.');
            }
          }
        } catch (e: any) {
          console.warn('[templates/custom] Error parsing structureJson:', e?.message);
          structureJson = null;
        }
      }
