...
export function WorkspaceDocumentEditor({
  ...
  const pageBreakdown = useMemo(() => {
    ...
    const hasExplicitPages = document.sections.some((s) => s.id.startsWith('sec-page-') || s.id.startsWith('sec-tpl-page-'));
    if (hasExplicitPages) {
    ...
    if (document.originalPageCount && document.originalPageCount > 1) {
      ...
      for (let i = 1; i <= document.originalPageCount; i++) {
        const matchingSec = document.sections.find((s) => s.id === `sec-page-${i}` || s.id === `sec-tpl-page-${i}`) || document.sections[i - 1];
        pagesArr.push({
          pageNumber: i,
  ...
  const parseMarkdownText = (text: string, baseKey: string) => {
    ...
    return lines.map((line, lIdx) => {
      ...
      const boldParts = contentText.split(/\*\*([^*]+)\*\*/g);
      const elements = boldParts.map((bPart, bIdx) => {
        if (bIdx % 2 === 1) {
          return <strong key={`b-${baseKey}-${lIdx}-${bIdx}`} style={{ fontWeight: 800 }}>{bPart}</strong>;
        }
        const italicParts = bPart.split(/\*([^*]+)\*/g);
        return italicParts.map((iPart, iIdx) => {
          if (iIdx % 2 === 1) {
            return <em key={`em-${baseKey}-${lIdx}-${bIdx}-${iIdx}`} style={{ fontStyle: 'italic' }}>{iPart}</em>;
          }
          return iPart;
        });
      });
      ...
      if (isListItem) {
        return (
          <div key={`line-${baseKey}-${lIdx}`} style={{ display: 'flex', alignItems: 'flex-start', paddingLeft: '16px', marginTop: '4px' }}>
            <span style={{ marginRight: '8px', color: '#0B2545', userSelect: 'none' }}>•</span>
            <span style={{ flex: 1 }}>{elements}</span>
          </div>
        );
      }
      return (
        <div key={`line-${baseKey}-${lIdx}`} style={{ minHeight: '1.2em' }}>
          {elements}
        </div>
      );
    });
  ...
  const renderVariables = (text: string, baseKey: number, blockStyle?: BlockStyle) => {
    ...
    return parts.map((part, idx) => {
      ...
      return parseMarkdownText(part, `seg-${baseKey}-${idx}`);
    });
  };
              ...
              >
                  ...
                  <div className="space-y-4 w-full max-w-full overflow-hidden break-words">
                    {activePageData?.sections?.map(({ section }) => {
                      ...
                      return (
                        ...
                        >
                          {/* Título del apartado */}
                          {!section.id.startsWith('sec-page-') && !section.id.startsWith('sec-tpl-page-') && (
                                ...
                                >
                                  {isEditing ? (
                                    ...
                                    <div className="flex items-start justify-between gap-2 w-full max-w-full overflow-hidden">
                                      <div className="my-2 overflow-x-auto w-full border border-slate-200 rounded-lg p-2 bg-slate-50/20 font-sans">
                                        <table className="w-full text-left border-collapse text-xs">
                                          {block.tableData.headers && (
                                            <thead>
                                              <tr className="border-b border-slate-300">
                                                {block.tableData.headers.map((h: any, hIdx: number) => (
                                                  <th key={hIdx} className="p-2 font-bold text-[#0B2545]">{h}</th>
                                                ))}
                                              </tr>
                                            </thead>
                                          )}
                                          <tbody>
                                            {block.tableData.rows?.map((row: any[], rIdx: number) => (
                                              <tr key={rIdx} className="border-b border-slate-100 last:border-0 hover:bg-slate-100/30">
                                      ...
                                      <button
                                        ...
                                        className="font-sans opacity-0 group-hover/block:opacity-100 p-1 bg-white border border-slate-300 hover:bg-slate-100 text-slate-600 text-xs rounded shadow-xs shrink-0 align-top mt-2"
                                        title="Editar tabla directamente"
                                      >
                                        ✏️
                                      </button>
                                    </div>
                ...
                <div className="border-t border-slate-200 pt-4 flex items-center justify-between text-[11px] text-slate-500 font-sans w-full">
                  <span className="font-semibold text-slate-700 truncate max-w-md">
                    {document.caseRefs?.expediente || document.title || ''}
                  </span>
