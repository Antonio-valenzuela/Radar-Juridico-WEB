        ...
        {filteredItems.length === 0 ? (
          <div className="py-10 px-4 text-center space-y-2 font-sans">
            <div className="text-2xl">
              {activeTab === 'docs' ? '📄' : activeTab === 'templates' ? '📋' : '📁'}
            </div>
            <p className="text-xs font-bold text-slate-500">
              {activeTab === 'docs'
                ? (searchQuery ? 'Sin resultados para esa búsqueda.' : 'Ningún documento en edición actualmente.')
                : activeTab === 'templates'
                ? (searchQuery ? 'Sin plantillas que coincidan.' : 'No tienes plantillas guardadas. Usa "Subir Machote" para agregar una.')
                : (searchQuery ? 'Sin documentos que coincidan.' : 'No hay documentos del expediente cargados. Sube un PDF o DOCX para comenzar.')}
            </p>
          </div>
        ) : (
