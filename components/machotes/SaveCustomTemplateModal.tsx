...
export function SaveCustomTemplateModal({
  ...
  const [structureJson, setStructureJson] = useState<any | null>(null);
  ...
  const processSelectedFile = async (selected: File) => {
    ...
    try {
      ...
      if (response.ok && payload.ok !== false) {
        ...
        setPageCount(pCount);
        setStructureJson(payload.structureJson ?? null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        if (structureJson) {
          formData.append('structureJson', JSON.stringify(structureJson));
        }
