...
- Jalisco: el portal oficial es `https://ciudadano.cjj.gob.mx/boletin_judicial/consultar`. Su interfaz pública consume `https://nilo.cjj.gob.mx/api/v1`; el adaptador sólo usa la ruta pública de búsqueda y un User-Agent identificable.
...
`PUBLISHED` significa que la fuente devolvió una coincidencia. `NOT_FOUND_AS_OF` sólo significa que no hubo coincidencias al corte registrado. `SOURCE_UNAVAILABLE`, `SOURCE_CHANGED`, `AUTH_REQUIRED`, `PENDING_RETRY` y `MANUAL_REVIEW` se muestran por separado; ningún error de red se interpreta como ausencia de publicación.
