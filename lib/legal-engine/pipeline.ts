...
Escribe la sección completa con desarrollo argumentativo exhaustivo.`;
      ...
      console.log(`[LegalEngine][Generate] PROVIDER_SELECTED: query to fast mode router`);
      const aiRes = await runFastMode({
        systemPrompt: 'Eres el Motor Forense de Análisis y Redacción Jurídica de Jurídico Radar.',
        userMessage: prompt,
        mode: 'fast',
      });
      console.log(`[LegalEngine][Generate] PROVIDER_RESPONSE: provider=${aiRes.provider}, model=${aiRes.model}, success=${aiRes.success}`);
      ...
      if (aiRes.success && aiRes.content) {
        ...
        console.error('[pipeline] AI call failed or returned empty content. Provider:', aiRes.provider, 'ErrorCode:', aiRes.errorCode, 'Warnings:', aiRes.warnings);
        aiError = aiRes.errorCode || (aiRes.warnings && aiRes.warnings[0]) || 'AI returned empty or failed.';
      }
    } catch (e: any) {
...
export async function runGenerationPipeline(
  ...
  try {
    ...
    const results = await pLimitMap(doc.sections, 2, async (section, i) => {
      ...
      console.log(`[pipeline] Generating section ${i + 1}/${doc.sections.length}: "${section.title}" (Type: ${section.type}) with concurrency limit...`);
      ...
      console.log(`[pipeline] Generated section ${i + 1}/${doc.sections.length} in ${duration}ms. aiUsed=${generated.aiUsed || false}, error=${generated.aiError || 'none'}`);
    ...
    for (const res of results) {
      if (!res) continue;
      const { sectionIndex, generated } = res;
