import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

async function main() {
  // Cleanup: remove the incorrectly-configured dof_jalisco record
  await prisma.officialSourceFetchLog.deleteMany({
    where: { source: { slug: 'dof_jalisco' } }
  });
  await prisma.officialSource.deleteMany({
    where: { slug: 'dof_jalisco' }
  });

  const sources = [
    {
      name: "Diario Oficial de la Federación (SIDOF)",
      slug: "SIDOF",
      baseUrl: "https://sidof.segob.gob.mx",
      healthUrl: "https://sidof.segob.gob.mx/apiStatus",
      adapter: "SIDOF",
      requiresBrowser: false,
      type: "sidof",
      jurisdiction: "Federal",
      country: "MX",
      description: "Ediciones matutinas, vespertinas y extraordinarias del DOF a través del API oficial de SIDOF.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "api",
      refreshFrequency: "daily",
    },
    {
      name: "Cámara de Diputados - Leyes Federales",
      slug: "DIPUTADOS",
      baseUrl: "https://www.diputados.gob.mx",
      healthUrl: "https://www.diputados.gob.mx/LeyesBiblio/index.htm",
      adapter: "DIPUTADOS",
      requiresBrowser: false,
      type: "diputados",
      jurisdiction: "Federal",
      country: "MX",
      description: "Portal oficial de la Cámara de Diputados de México que contiene el compendio actualizado de leyes federales vigentes.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "html",
      refreshFrequency: "weekly",
    },
    {
      name: "SCJN - Semanario Judicial de la Federación",
      slug: "SCJN_SJF",
      baseUrl: "https://sjf2.scjn.gob.mx",
      healthUrl: "https://sjf2.scjn.gob.mx/",
      adapter: "SJF",
      requiresBrowser: true,
      type: "sjf",
      jurisdiction: "Federal",
      country: "MX",
      description: "Tesis, jurisprudencias y sentencias del Semanario Judicial de la Federación (SCJN).",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "api",
      refreshFrequency: "daily",
    },
    {
      name: "SCJN - Legislación de la Suprema Corte",
      slug: "SCJN_LEG",
      baseUrl: "https://legislacion.scjn.gob.mx",
      healthUrl: "https://legislacion.scjn.gob.mx/buscador/paginas/buscar.aspx",
      adapter: "SCJN_LEG",
      requiresBrowser: true,
      type: "scjn",
      jurisdiction: "Federal",
      country: "MX",
      description: "Compendio legal, reformas y normativas integradas en el portal de legislación de la Suprema Corte.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "api",
      refreshFrequency: "weekly",
    },
    {
      name: "Diario Oficial de la Federación (Web)",
      slug: "DOF_WEB",
      baseUrl: "https://www.dof.gob.mx",
      healthUrl: "https://www.dof.gob.mx",
      adapter: "DOF",
      requiresBrowser: false,
      type: "dof",
      jurisdiction: "Federal",
      country: "MX",
      description: "Página web principal del Diario Oficial de la Federación (DOF) para crawling HTML alternativo.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "html",
      refreshFrequency: "daily",
    },
    {
      name: "Cámara de Diputados - CNPCF",
      slug: "cnpcf_diputados",
      baseUrl: "https://www.diputados.gob.mx/LeyesBiblio/pdf/CNPCF.pdf",
      healthUrl: "https://www.diputados.gob.mx/LeyesBiblio/pdf/CNPCF.pdf",
      adapter: "DIPUTADOS",
      requiresBrowser: false,
      type: "diputados",
      jurisdiction: "Federal",
      country: "MX",
      matter: "civil,familiar,procesal",
      description: "Texto oficial en PDF del Código Nacional de Procedimientos Civiles y Familiares publicado por la Cámara de Diputados.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "manual_url",
      refreshFrequency: "weekly",
    },
    {
      name: "CJF - SISE",
      slug: "cjf_sise",
      baseUrl: "https://sise.cjf.gob.mx/SiseInternet/default.aspx",
      healthUrl: "https://sise.cjf.gob.mx/SiseInternet/default.aspx",
      adapter: "GENERIC_HTML",
      requiresBrowser: true,
      type: "court",
      jurisdiction: "Federal",
      country: "MX",
      matter: "judicial",
      description: "Sistema Integral de Seguimiento de Expedientes del Consejo de la Judicatura Federal; requiere navegador o sesión según consulta.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "search_only",
      refreshFrequency: "daily",
    },
    {
      name: "CJF - Consultas y servicios",
      slug: "boletin_general_cjf",
      baseUrl: "https://www.cjf.gob.mx/consultas.htm",
      healthUrl: "https://www.cjf.gob.mx/consultas.htm",
      adapter: "GENERIC_HTML",
      requiresBrowser: false,
      type: "court",
      jurisdiction: "Federal",
      country: "MX",
      matter: "judicial",
      description: "Página de consultas del Consejo de la Judicatura Federal: normativa, listas de acuerdos, expedientes, criterios y sentencias públicas.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "search_only",
      refreshFrequency: "daily",
    },
    {
      name: "Boletín Judicial Federal - CJF",
      slug: "boletin_judicial_federal",
      baseUrl: "https://www.dgej.cjf.gob.mx/SiseInternet/consulta/busquedaacuerdos.aspx",
      healthUrl: "https://www.dgej.cjf.gob.mx/SiseInternet/consulta/busquedaacuerdos.aspx",
      adapter: "GENERIC_HTML",
      requiresBrowser: true,
      type: "court",
      jurisdiction: "Federal",
      country: "MX",
      matter: "judicial",
      description: "Consulta federal de listas/acuerdos del Poder Judicial Federal; se mantiene como búsqueda externa por restricciones de navegación.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "search_only",
      refreshFrequency: "daily",
    },
    {
      name: "Periódico Oficial del Estado de Jalisco",
      slug: "PERIODICO_OFICIAL_JALISCO",
      baseUrl: "https://periodicooficial.jalisco.gob.mx",
      healthUrl: "https://apiperiodico.jalisco.gob.mx/api/newspaper/public?fecha=&search=&page=1&perPage=1",
      adapter: "PERIODICO_OFICIAL_JALISCO",
      requiresBrowser: false,
      type: "state_gazette",
      jurisdiction: "Jalisco",
      country: "MX",
      state: "Jalisco",
      matter: "estatal",
      description: "Periódico Oficial El Estado de Jalisco; índice público por fecha y ejemplares electrónicos en PDF.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "api",
      refreshFrequency: "daily",
    },
    {
      name: "Boletín Judicial del Estado de Jalisco",
      slug: "boletin_judicial_jalisco",
      baseUrl: "https://ciudadano.cjj.gob.mx/boletin_judicial/consultar",
      healthUrl: "https://ciudadano.cjj.gob.mx/boletin_judicial/consultar",
      adapter: "JALISCO_BULLETIN",
      requiresBrowser: false,
      type: "state_gazette",
      jurisdiction: "State",
      country: "MX",
      state: "Jalisco",
      matter: "judicial",
      description: "Boletín judicial oficial del Consejo de la Judicatura del Estado de Jalisco.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "api",
      refreshFrequency: "daily",
    },
    {
      name: "Tribunal de Justicia Administrativa de Jalisco - Boletines",
      slug: "boletin_tjajal",
      baseUrl: "https://tjajal.gob.mx/boletines",
      healthUrl: "https://tjajal.gob.mx/boletines",
      adapter: "TJAJAL_BULLETIN",
      requiresBrowser: false,
      type: "court",
      jurisdiction: "State",
      country: "MX",
      state: "Jalisco",
      matter: "administrativo",
      description: "Punto de extensión para boletines del Tribunal de Justicia Administrativa de Jalisco; requiere revisión manual mientras no exista una API pública estable.",
      isActive: true,
      isOfficial: true,
      trustLevel: "official",
      crawlMode: "search_only",
      refreshFrequency: "daily",
    }
  ];

  console.log("Seeding official sources...");

  for (const src of sources) {
    const existing = await prisma.officialSource.findFirst({
      where: { slug: { equals: src.slug, mode: "insensitive" } },
    });

    if (existing) {
      await prisma.officialSource.update({ where: { id: existing.id }, data: src });
    } else {
      await prisma.officialSource.create({ data: src });
    }
  }

  console.log(`Seeding complete! ${sources.length} official sources registered.`);
}

main()
  .catch((e) => {
    console.error(e);
    process.exit(1);
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
